...
function Banque() {
  ...
  useEffect(() => {
    supabase.from('social_contenus').select('*').order('updated_at',{ascending:false})
  ...
  async function markPublished(item) {
    ...
    setItems(prev => prev.map(i => i.id===item.id ? {...i,statut:'publie'} : i))
    setSelected(null)
  }

  const counts = {}
  STATUTS.forEach(s => counts[s.key] = items.filter(i=>i.statut===s.key).length)
  ...
  return (
    <div>
      ...
      {filtered.length===0
        ? <div style={{textAlign:'center',padding:40,color:'#94a3b8',fontStyle:'italic'}}>Aucun contenu</div>
        : <div style={{display:'flex',flexDirection:'column',gap:8}}>
            {filtered.map(item => {
              ...
              return (
                <div key={item.id} onClick={()=>setSelected(item)} style={{background:'#fff',border:'1px solid #DDD5B8',borderRadius:12,padding:'12px 14px',cursor:'pointer',display:'flex',gap:12,alignItems:'center'}}>
                  <span style={{fontSize:20}}>{plat.icon}</span>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontWeight:700,fontSize:13,color:'#0D1520',marginBottom:4}}>{item.titre}</div>
                    <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
                      <Chip color={theme.color} bg={theme.bg}>{theme.label}</Chip>
                      <Chip color={statut.color} bg={statut.bg}>{statut.label}</Chip>
                    </div>
                  </div>
                  {item.date_publi&&<div style={{fontSize:11,color:'#94a3b8',whiteSpace:'nowrap'}}>{new Date(item.date_publi).toLocaleDateString('fr-FR')}</div>}
                </div>
              )
            })}
          </div>
  ...
  return (
    <div>
      <div style={{display:'flex',gap:6,marginBottom:16}}>
        {[{key:'all',label:'Tous'},...PLATES.filter(p=>p.key!=='les_deux')].map(p=>(
          <button key={p.key} onClick={()=>setFilter(p.key)} style={{...s.fBtn,...(filter===p.key?s.fBtnActive:{})}}>
            {p.icon} {p.label}
          </button>
        ))}
      </div>
      ...
      {filtered.length===0
        ...
        : <div style={{display:'flex',flexDirection:'column',gap:8}}>
            {filtered.map(stat => {
              const plat = plateOf(stat.plateforme)
              ...
              return (
                <div key={stat.id} style={{background:'#fff',border:'1px solid #DDD5B8',borderRadius:12,padding:'12px 14px',display:'flex',alignItems:'center',gap:12}}>
                  <span style={{fontSize:20}}>{plat.icon}</span>
                  <div style={{flex:1}}>
                    <div style={{fontWeight:700,fontSize:13,color:'#0D1520'}}>{stat.titre||'—'}</div>
